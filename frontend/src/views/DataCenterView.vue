...
<template>
  <div class="page">
    <PageState :loading="loading && !snapshot" :error="error" @retry="refresh">
      <div class="page-intro"><div><p>DATA EXCHANGE</p><h2>数据导入与导出</h2><span>通过标准模板更新完整仓库数据集，或导出可再次导入的原始明细。</span></div><div class="snapshot-note"><Database :size="19" /><div><strong>当前数据集</strong><small>{{ snapshot?.meta?.period }} · {{ snapshot?.trend?.length || 0 }} 个业务日</small></div></div></div>
      <section class="data-center-grid">
        <PanelCard title="导入运营数据" subtitle="支持完整 Excel 工作簿和仓库日指标 CSV" eyebrow="IMPORT DATA">
          <div class="upload-zone" :class="{ active: dragActive, ready: accepted }" @dragenter.prevent="dragActive = true" @dragover.prevent="dragActive = true" @dragleave.prevent="dragActive = false" @drop.prevent="onDrop" @click="fileInput?.click()">
            ...
            <template v-else><strong>拖放文件到此处，或点击选择</strong><span>Excel 事务替换完整数据集；CSV 按日期与仓库合并</span></template>
          </div>
        ...
        <PanelCard title="导出与交付" subtitle="按不同使用场景生成文件" eyebrow="EXPORT DATA">
          <div class="export-options">
            <button @click="exportData('xlsx')"><span class="export-icon excel"><FileSpreadsheet :size="24" /></span><div><strong>完整运营工作簿</strong><small>包含主数据、库存、日指标、库区、异常、BOM 和目标 8 个工作表</small></div><Download :size="18" /></button>
          ...
          <div class="data-governance"><h3>数据口径保障</h3><div><span>01</span><p><strong>事务化完整导入</strong>八类数据全部校验通过后统一提交。</p></div><div><span>02</span><p><strong>字段级校验</strong>日期、主键与数值格式不合法时终止导入。</p></div><div><span>03</span><p><strong>统一导出口径</strong>导出始终使用当前 MySQL 有效数据。</p></div></div>
        </PanelCard>
      </section>
    </PageState>
  </div>
