...
CREATE TABLE IF NOT EXISTS inventory_age_rule (
  ...
  action_guidance VARCHAR(500),
  applicable_scope VARCHAR(100),
  PRIMARY KEY (rule_type, rule_name)
);

CREATE TABLE IF NOT EXISTS inventory_age_batch (
  snapshot_date DATE NOT NULL,
  ...
  outbound_qty_30d DECIMAL(20,4) NOT NULL DEFAULT 0,
  outbound_rate_30d DECIMAL(12,6),
  coverage_days DECIMAL(20,2),
  movement_status VARCHAR(50),
  stagnant_level VARCHAR(50) NOT NULL,
  is_stagnant BOOLEAN NOT NULL DEFAULT FALSE,
  stagnant_score DECIMAL(10,2) NOT NULL DEFAULT 0,
  priority VARCHAR(30),
  recommended_action VARCHAR(500),
  owner VARCHAR(200),
  data_source VARCHAR(200),
  PRIMARY KEY (snapshot_date, age_batch_id),
  CONSTRAINT fk_age_batch_warehouse FOREIGN KEY (warehouse_id) REFERENCES warehouse (warehouse_id),
  INDEX idx_age_batch_warehouse_date (warehouse_id, snapshot_date),
  INDEX idx_age_batch_project_sku (project_material_sku),
  INDEX idx_age_batch_stagnant (is_stagnant, stagnant_level, priority)
);

CREATE TABLE IF NOT EXISTS inventory_age_sku (
  snapshot_date DATE NOT NULL,
  warehouse_id VARCHAR(32) NOT NULL,
  warehouse_name VARCHAR(100) NOT NULL,
  warehouse_type VARCHAR(50) NOT NULL,
  ...
  on_hand_qty DECIMAL(20,4) NOT NULL DEFAULT 0,
  available_qty DECIMAL(20,4) NOT NULL DEFAULT 0,
  inventory_amount DECIMAL(20,2),
  weighted_avg_age_days DECIMAL(12,2),
  max_age_days INT NOT NULL DEFAULT 0,
  dominant_age_bucket VARCHAR(30),
  outbound_qty_30d DECIMAL(20,4) NOT NULL DEFAULT 0,
  outbound_rate_30d DECIMAL(12,6),
  latest_sku_outbound_date DATE,
  days_since_last_sku_outbound INT,
  stagnant_batch_count INT NOT NULL DEFAULT 0,
  stagnant_inventory_amount DECIMAL(20,2),
  stagnation_ratio DECIMAL(12,6),
  stagnant_level VARCHAR(50) NOT NULL,
  is_stagnant BOOLEAN NOT NULL DEFAULT FALSE,
  stagnant_score DECIMAL(10,2) NOT NULL DEFAULT 0,
  priority VARCHAR(30),
  recommended_action VARCHAR(500),
  owner VARCHAR(200),
  PRIMARY KEY (snapshot_date, warehouse_id, project_material_sku),
  CONSTRAINT fk_age_sku_warehouse FOREIGN KEY (warehouse_id) REFERENCES warehouse (warehouse_id),
  INDEX idx_age_sku_warehouse_date (warehouse_id, snapshot_date),
  INDEX idx_age_sku_project (project_no),
  INDEX idx_age_sku_stagnant (is_stagnant, stagnant_level, priority)
);
